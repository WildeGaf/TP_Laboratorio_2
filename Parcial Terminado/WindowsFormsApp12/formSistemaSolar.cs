...
namespace WindowsFormsApp12
{
  public partial class formSistemaSolar : Form
  {
    ...
    public void BtnAgregarPlaneta_Click(object sender, EventArgs e)
    {
      ...
      {
        Planeta p = new Planeta(orbitar, (int)numRotacion.Value, txtNombrePlaneta.Text, (int)numSatelite.Value, (Tipo)cmbTipo.SelectedIndex);
        if (planetas.Count != 0)
        {
    ...
    private void BtnAgregarSatelite_Click(object sender, EventArgs e)
    {
          ...
          numRotacionSatelite.Value > 0)
      {
        ...
        {
          if ((string)planetas[i] == cmbPlanetas.Text)
          {
            resp = (Astro)satelite + (Planeta)planetas[i];
          }
        }
      }
